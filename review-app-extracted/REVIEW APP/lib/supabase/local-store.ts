...
export class LocalStore implements DataStore {
  ...
  async getCurrentUser(): Promise<User | null> {
    ...
    const u = db.users.find((u) => u.id === db.currentUserId) ?? null;
    if (!u) return null;
    return { ...u, role: u.role ?? "individual" };
  }
  ...
  async getOrganizationsForUser(userId: string): Promise<Organization[]> {
    const db = await read();
    return db.organizations.filter((o) => o.owner_user_id === userId);
  }
  ...
  async updateOrganization(id: string, patch: Partial<Organization>): Promise<Organization> {
    return serialize(async () => {
      ...
      const current = notNull(db.organizations[idx], `organization ${id} not found`);
  ...
  async getBusinessesForUser(userId: string): Promise<Business[]> {
    const db = await read();
    return db.businesses.map(b => ({ ...b, organization_id: b.organization_id ?? null })).filter((b) => b.owner_user_id === userId);
  }

  async getBusinessesForOrganization(organizationId: string): Promise<Business[]> {
    const db = await read();
    return db.businesses.map(b => ({ ...b, organization_id: b.organization_id ?? null })).filter((b) => b.organization_id === organizationId);
  }

  async listConnectedBusinesses(): Promise<Business[]> {
    const db = await read();
    return db.businesses.map(b => ({ ...b, organization_id: b.organization_id ?? null })).filter(
      (b) =>
  ...
  async getBusinessById(id: string): Promise<Business | null> {
    const db = await read();
    const b = db.businesses.find((b) => b.id === id) ?? null;
    if (!b) return null;
    return { ...b, organization_id: b.organization_id ?? null };
  }
