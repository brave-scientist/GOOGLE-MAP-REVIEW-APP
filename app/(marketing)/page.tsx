 ...
 *
 * Faithful port of landing-page.html: exact copy, color tokens, typography,
 * section order, the animated hero demo card (typewriter reply + posted check),
 * and FAQ accordion. All design tokens come from tailwind.config.ts which
 * matches the :root CSS variables in the original HTML.
 */
  ...
  return (
    <div className="bg-ink text-cream overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 border-b border-cream/10 bg-ink/90 backdrop-blur">
        <nav className="mx-auto flex max-w-[1180px] items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="#pricing" className="rounded-lg bg-brass px-5 py-2.5 text-sm font-semibold text-ink transition hover:-translate-y-px hover:shadow-lg">Start free trial</a>
          </div>
        </nav>
      </header>
      ...
      <section className="py-20">
        <div className="mx-auto grid max-w-[1180px] grid-cols-1 gap-14 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="font-display text-4xl font-bold leading-[1.06] lg:text-[52px]">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3.5">
              <a href="#pricing" className="rounded-lg bg-brass px-7 py-3.5 text-base font-semibold text-ink transition hover:-translate-y-px hover:shadow-lg">Start your 14-day free trial</a>
              <a href="#how" className="rounded-lg border-[1.5px] border-cream/30 px-7 py-3.5 text-base font-semibold text-cream transition hover:border-brass hover:text-brass-light">See how it works</a>
            </div>
            <p className="mt-4 text-sm text-cream-dim">No setup call needed · Cancel anytime · Card required to start trial</p>
          </div>

          {/* Demo card */}
          <div className="rounded-[18px] border border-cream/10 bg-ink-2 p-5 shadow-2xl">
      ...
      <section className="bg-paper py-20 text-ink-text">
        <div className="mx-auto max-w-[1180px] px-6">
          ...
          <div className="grid grid-cols-1 gap-7 sm:grid-cols-3">
            <StatCard num="62%" label="of shoppers read a business review replies before deciding to visit" tag="Placeholder stat — replace with sourced figure" />
            <StatCard num="4+ days" label="average time an independent business takes to reply to a review" tag="Placeholder stat — replace with sourced figure" />
            <StatCard num="$0" label="extra hours in your week to fix this manually" />
          </div>
        </div>
      </section>
      ...
      <section id="pricing" className="py-20">
        <div className="mx-auto max-w-[1180px] px-6">
          ...
          <div className="mx-auto grid max-w-3xl grid-cols-1 gap-7 sm:grid-cols-2">
            <PriceCard plan="Starter" price="$29" desc="For a single location just getting review replies under control." features={["1 location", "Google review replies (AI-drafted)", "Quick-add + paste-list review requests", "Email review requests", "100 review requests included / month", "Email support"]} cta="Start free trial" featured={false} />
            <PriceCard plan="Pro" price="$59" desc="For businesses that want Facebook covered and text-message requests." features={["1 location", "Google + Facebook review replies", "All review-request entry methods", "SMS + email review requests", "300 review requests included / month", "Priority support"]} cta="Start free trial" featured={true} />
          </div>
      ...
      <section className="bg-paper py-20 text-ink-text">
        <div className="mx-auto max-w-[1180px] px-6">
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            {[0, 1, 2].map((i) => (
              <div key={i} className="rounded-xl bg-paper-2 p-6">
                <div className="mb-3 text-base tracking-wider text-coral">★★★★★</div>
                <p className="mb-4 text-[15px] leading-relaxed text-[#33422F]">&ldquo;Placeholder quote — swap with a real testimonial collected during beta. Keep it specific and short, not generic praise.&rdquo;</p>
                <div className="flex items-center gap-2.5">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full border-[1.5px] border-dashed border-brass bg-paper font-mono text-[10px] text-[#8a7a4d]">PHOTO</div>
                  <div>
                    <div className="text-sm font-semibold text-ink-text">[Owner Name]</div>
                    <div className="text-xs text-[#6A7A6F]">[Business name, city]</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="pb-0">
        <div className="mx-6 mb-0 rounded-3xl bg-ink-2 px-6 py-20 text-center">
          ...
          <a href="#pricing" className="inline-block rounded-lg bg-brass px-7 py-3.5 text-base font-semibold text-ink transition hover:-translate-y-px hover:shadow-lg">Start your 14-day free trial</a>
        </div>
      </section>
      ...
      <footer className="px-6 pb-9 pt-14">
        <div className="mx-auto max-w-[1180px]">
          <div className="mb-11 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
            ...
            <FooterCol title="Product" links={[{ href: "#how", label: "How it works" }, { href: "#features", label: "Features" }, { href: "#pricing", label: "Pricing" }, { href: "#faq", label: "FAQ" }]} />
...
function StatCard({ num, label, tag }: { num: string; label: string; tag?: string }) {
  return (
...
function StepCard({ num, title, desc }: { num: string; title: string; desc: string }) {
  return (
    <div className="rounded-xl border border-cream/5 bg-ink-2 p-6">
...
function PriceCard({ plan, price, desc, features, cta, featured }: { plan: string; price: string; desc: string; features: string[]; cta: string; featured: boolean }) {
  return (
    <div className={`rounded-2xl p-9 ${featured ? "border-2 border-brass bg-ink-2" : "border border-cream/10 bg-ink-2"}`}>
      ...
      <div className="mb-0.5 font-mono text-5xl font-semibold">{price} <span className="text-base font-normal text-cream-dim">/ month</span></div>
      <p className="mb-5 text-sm text-cream-dim">{desc}</p>
      ...
      <a href="/login" className={`block w-full rounded-lg py-2.5 text-center text-sm font-semibold transition ${featured ? "bg-brass text-ink hover:brightness-110" : "border border-cream/30 text-cream hover:border-brass hover:text-brass-light"}`}>{cta}</a>
    </div>
  );
}
