...
export default function SignupPage() {
  ...
  const [selectedPlan] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (qPlan) {
        const upper = qPlan.trim().toUpperCase()
        if (['FREE', 'STARTER', 'PRO', 'ENTERPRISE', 'AGENCY'].includes(upper)) {
          return upper
        }
      }
    }
          ...
          {step === 1 && (
            <div className="mb-4">
              ...
              <div className="flex items-center gap-3 my-4">
                ...
                <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider">or with email</span>
                <div className="flex-1 h-px bg-border/40" />
              </div>
            </div>
          )}
