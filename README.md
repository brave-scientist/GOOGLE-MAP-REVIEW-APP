...
| `FACEBOOK_APP_ID` / `FACEBOOK_APP_SECRET` | Optional | Facebook OAuth credentials for Facebook Pages integration. Get from developers.facebook.com → My Apps. Requires App Review for `pages_read_engagement` and `pages_manage_engagement` permissions (2-4 weeks). | Facebook integration disabled; `/api/oauth/facebook` returns 503 with setup instructions. |
